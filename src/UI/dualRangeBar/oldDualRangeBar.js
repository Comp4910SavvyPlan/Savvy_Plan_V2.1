...
class DualRangeRangeBar extends Component {
  ...
  handleInputChange(e) {
    ...
    return e.target.value.length > 1
      ? this.props.setKeyVariables(e.target.name, +e.target.value)
      : null;
  }
  ...
  handleClickRight = () => {
    ...
    const toAge =
      this.state.toAge + 10 < 95
        ? this.state.toAge + this.state.skipForwardBy
        : 95;
    console.log(fromAge);
    this.setState({
      fromAge: fromAge,
  ...
  render() {
    ...
    const percentageMin =
      this.state.fromAge.length > 1 || this.state.fromAge.length === undefined
        ? this.state.fromAge / (95 - this.props.userAge)
        : null; //percentage of the runnerbar that is shifting left as the thumb moves
    ...
    const fillBarWidth = (
      (percentageMax - percentageMin) * totalWidth +
      1
    ).toFixed(); // how long the div will be

    return (
      <div>
        {/* this is the runner bar*/}
        <InputWrapper totalWidth={totalWidth}>
        ...
        <ValueBoxWrapper>
          {this.state.lowerValueAsInput ? (
            ...
            <Value onClick={() => this.toggleLowerValueAsInputState()}>
              {this.state.fromAge.toLocaleString()}
            </Value>
          )}
          ...
          {this.state.higherValueAsInput ? (
            ...
            <Value onClick={() => this.toggleHigherValueAsInputState()}>
              {this.state.toAge}
            </Value>
          )}
        </ValueBoxWrapper>
